...
function onLoad2(event) {
  /*(function () {
    const contentSection = localStorage.getItem("content-section");
    item = document.querySelector(`#${contentSection}`);
    if (contentSection) {
      showSection(contentSection, defaultNavItem);
    }
  })();*/
  (function () {
    const contentSection = localStorage.getItem("contentSection") || "home";
...
function setTheme(themeName) {
  document
    ...
    .setAttribute("href", `css/framework/themes/w3-theme-${themeName}.css`);
...
function showSection(sectionId, navItem) {
  ...
  // Reset navigation bar SVGs to default
  const navItems = document.querySelectorAll(".w3-bar .w3-bar-item");
  navItems.forEach((item) => {
    if (item.classList.contains("active")) {
      item.classList.remove("active");
      if (item.dataset.iconDefault) {
        item
          .querySelector("svg use")
          .setAttribute(
            "href",
            `/icons/sprite.svg#${item.dataset.iconDefault}`
          );
      }
    }
  });
  ...
  // Set SVG to active
  if (navItem.dataset.iconActive) {
    navItem
      .querySelector("use")
      .setAttribute("href", `/icons/sprite.svg#${navItem.dataset.iconActive}`);
  }
  localStorage.setItem("contentSection", sectionId);
}
...
function onMessage(event) {
  ...
  // Date and time
    ...
    document.getElementById("relayTime").innerHTML =
      data.date + " " + data.time;
  }
  ...
  // Sensors
    ...
    if (data.moisture != "No sensor") {
      document.getElementById("moistureSensor").innerHTML =
        " Humidity <b>" + data.moisture + "</b>%";
    } else {
    ...
    if (data.flow != "No sensor") {
      document.getElementById("flowSensor").innerHTML =
        " Flow <b>" + data.flow + "</b> l/min";
    } else {
      document.getElementById("flowSensor").innerHTML = " No sensor";
    }

    // Seasonal adjustment
    document.getElementById("seasonAdjustment").innerHTML =
      " Seasonnal adj. <b>" + data.adjustment + "</b>%";
  }
  ...
  // Manual mode -> Update timers
    ...
    for (let i = 0; i < ways.length; i++) {
      ...
      if (token[1] != "00:00") {
        ...
        document.getElementById(id_btn).innerHTML =
          '<i class="fa-solid fa-play" aria-hidden="true"></i>'; // Toggle button to START
      }
    }
  }
  ...
  // Watering status -> Next irrigation / Watering in progress / Manual mode / No scheduled watering
    ...
    if (
      data.status == "Manual watering" ||
      data.status == "Watering in progress"
    ) {
      ...
      for (let i = 0; i < infos.length; i++) {
        ...
        document.getElementById("wateringLine2").innerHTML =
          token[1] + " min remaining";
        document.getElementById("wateringLine3").innerHTML =
          infos.length > 1
            ? "<i><small>" +
              (infos.length - 1) +
              " more in progress</small></i>"
            : "";
      }
      // Next watering and no scheduled irrigation
    } else {
      document.getElementById("wateringLine1").innerHTML =
