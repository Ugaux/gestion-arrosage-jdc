...
const uint8_t SPIFFSIniFile::maxFilenameLen = SPIFFSINIFILE_MAX_FILENAME_LEN;
...
bool SPIFFSIniFile::getValue(const char* section, const char* key,
  ...
  if (strcasecmp(buffer, "true") == 0 || strcasecmp(buffer, "yes") == 0 || strcasecmp(buffer, "1") == 0) {
    val = true;
    return true;
  }
  if (strcasecmp(buffer, "false") == 0 || strcasecmp(buffer, "no") == 0 || strcasecmp(buffer, "0") == 0) {
    val = false;
    return true;
  }
