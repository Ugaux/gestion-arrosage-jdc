...
void setup(void) {
  Wire.setClock(400000);  // TODO: here
  ...
  // TODO: HERE
  if (not DEV_MODE) {
    Serial.printf("\nCréation du point d'accès avec SSID:%s et PWD:%s ...\n", iniConfig.getSSID(), iniConfig.getPassword());
    if (not WiFi.softAP(iniConfig.getSSID(), NULL)) {
    ...
    Serial.print("Connection à OnlyJesusSaves wifi...");
    WiFi.begin("OnlyJesusSaves", "JCIMLFE1993");
...
void loop() {
  ...
  // available stack size, If the returned value is very low (e.g., < 200 bytes),
  // you are close to a crash and should increase the size using the macro above.
  /*
  // Set stack size to 16KB (must be placed before setup())
