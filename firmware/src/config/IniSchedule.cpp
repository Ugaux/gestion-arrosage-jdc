#include "IniSchedule.h"

#include <LittleFS.h>

IniSchedule::IniSchedule(const char *fileName) : SPIFFSIniFile(fileName) {
  m_fileName = fileName;
}

// Read the schedule configuration file
bool IniSchedule::read(void) {
  if (!open()) {
...
bool IniSchedule::write(void) {
  ...
  while (way != 0 && way->getName() != 0) {
    Serial.printf("IniSchedule::write: %s\n", way->getName());
    file.printf("\n[%s]\n", way->getName());
    for (int i = 0; i < MAX_SCHEDULE; i++) {
...
String IniSchedule::formatFrequency(Watering *w) {
