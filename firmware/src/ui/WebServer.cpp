...
void handleAdd(AsyncWebServerRequest *request) {
  ...
  request->send(LittleFS, "/www/pages/add.html", "text/html", false, templateProcessor);
}
...
void handleEdit(AsyncWebServerRequest *request) {
  ...
  request->send(LittleFS, "/www/pages/edit.html", "text/html", false, templateProcessor);
}
  ...
  while (z != 0) {
    ...
    while (way != 0) {
      ...
      if (lastZone == way->getZone()->getName()) {
        ...
        way->getName();
        way->getShortName();
        way->manualStarted(NULL);
        Watering::getManualDuration();
        ...
        for (int i = 0; i < MAX_WATERING; i++) {
          ...
          if (w->getDuration() != 0) {
            if (lastWay == w->getWayName()) {
              schedule;
              w->getWayName();                  // Enable switch
              w->getIndex();                    // Enable switch
              w->getEnable() ? "checked" : "";  // Enable switch
