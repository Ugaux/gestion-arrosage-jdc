...
void test_validate_default_config(void) {
  ...
  TEST_ASSERT_TRUE_MESSAGE(
    res.ok(), "The default config should pass validation");
  TEST_ASSERT_TRUE_MESSAGE(
    res.path() == "",
    "No path should be present if config passed validation");
  TEST_ASSERT_TRUE_MESSAGE(
    res.message() == "",
    "No error message should be present if config passed validation");

  cfg.userSettings.params.watering.seasonal.factor = 0;

  res = validateConfig(cfg);
  TEST_ASSERT_TRUE_MESSAGE(
    !res.ok(), "A bad default config should fail validation");
  TEST_ASSERT_TRUE_MESSAGE(
    res.path() != "",
    "A path should be present if config failed validation");
  TEST_ASSERT_TRUE_MESSAGE(
    res.message() != "",
    "An error message should be present if config failed validation");
}
